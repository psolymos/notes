...
### Scenario 2: Fill up when cheaper
...
### Scenario 3: Don't want to fill up every day
...
```{r scen4}
fun_pricediff <- function(pricediff=0.01) {
    z$refill <- FALSE # indicates decision to refill
    z$refill[1] <- TRUE
    z$distance <- 0
    z$level <- vol # gas in tank in L
    ...
    for (i in 2:n) {
        ...
        doRefill1 <- z$level[i-1] - LperD < 0
        z$dsf[i] <- z$day[i] - z$day[rev(which(z$refill))[1]]
        doRefill2 <- (1-pricediff)*z$price[i-1] > z$price[i]
        if (doRefill1 || doRefill2) { # refill
            z$level[i] <- z$level[i-1] - LperD
            z$refill[i] <- TRUE
            z$cost[i] <- z$price[i] * (vol - z$level[i])
...
Diff <- 1:10/100
res2 <- lapply(Diff, fun_pricediff)
Savings2 <- sapply(res2, function(z) sum(z$cost) - sum(z1$cost))
Times2 <- sapply(res2, function(z) sum(z$refill) / sum(z1$refill))
plot(Diff, Savings2, type="l")
abline(h=0, lty=2)
plot(Diff, Times2, type="l")
abline(h=1, lty=2)
```
